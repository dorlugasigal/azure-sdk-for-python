...
try:
    ...
    @_evee_evaluator(name='tool_call_success')
    class _ToolCallSuccessEvaluatorEveeEvaluator(_EveeBaseEvaluator):
        """Bridge: real _ToolCallSuccessEvaluator registered as evee @evaluator."""
        def __init__(self, cloud_config=None, deployment_name=None, context=None, **kwargs):
            ...
            self._evaluator = _ToolCallSuccessEvaluator(model_config=model_config) if model_config else None
        ...
        def compute(self, query='', response='', tool_definitions=None, tool_calls=None, **kwargs):
            if not self._evaluator:
                return {"tool_call_success": 0.0, "tool_call_success_result": "error", "tool_call_success_reason": "No model config"}
            try:
                ...
                return {"tool_call_success": 0.0, "tool_call_success_result": "error", "tool_call_success_reason": str(e)}

        def aggregate(self, scores):
            vals = [s.get("tool_call_success", 0) for s in scores if isinstance(s.get("tool_call_success"), (int, float))]
            return {"tool_call_success_mean": round(sum(vals) / len(vals), 4)} if vals else {}
except ImportError:
