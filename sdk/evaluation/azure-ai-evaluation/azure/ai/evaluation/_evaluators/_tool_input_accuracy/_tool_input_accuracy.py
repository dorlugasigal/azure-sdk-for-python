...
try:
    ...
    @_evee_evaluator(name='tool_input_accuracy')
    class _ToolInputAccuracyEvaluatorEveeEvaluator(_EveeBaseEvaluator):
        """Bridge: real _ToolInputAccuracyEvaluator registered as evee @evaluator."""
        def __init__(self, connections_registry=None, context=None, **kwargs):
            ...
            self._evaluator = _ToolInputAccuracyEvaluator(model_config=model_config) if model_config else None
        ...
        def compute(self, query='', response='', tool_definitions=None, tool_calls=None, **kwargs):
            if not self._evaluator:
                return {"tool_input_accuracy": 0.0, "tool_input_accuracy_result": "error", "tool_input_accuracy_reason": "No model config"}
            try:
                ...
                return {"tool_input_accuracy": 0.0, "tool_input_accuracy_result": "error", "tool_input_accuracy_reason": str(e)}

        def aggregate(self, scores):
            vals = [s.get("tool_input_accuracy", 0) for s in scores if isinstance(s.get("tool_input_accuracy"), (int, float))]
            return {"tool_input_accuracy_mean": round(sum(vals) / len(vals), 4)} if vals else {}
except ImportError:
