...
try:
    ...
    @_evee_evaluator(name='tool_output_utilization')
    class _ToolOutputUtilizationEvaluatorEveeEvaluator(_EveeBaseEvaluator):
        """Bridge: real _ToolOutputUtilizationEvaluator registered as evee @evaluator."""
        def __init__(self, connections_registry=None, context=None, **kwargs):
            ...
            self._evaluator = _ToolOutputUtilizationEvaluator(model_config=model_config) if model_config else None
        ...
        def compute(self, query='', response='', tool_definitions=None, tool_calls=None, **kwargs):
            if not self._evaluator:
                return {"tool_output_utilization": 0.0, "tool_output_utilization_result": "error", "tool_output_utilization_reason": "No model config"}
            try:
                ...
                return {"tool_output_utilization": 0.0, "tool_output_utilization_result": "error", "tool_output_utilization_reason": str(e)}

        def aggregate(self, scores):
            vals = [s.get("tool_output_utilization", 0) for s in scores if isinstance(s.get("tool_output_utilization"), (int, float))]
            return {"tool_output_utilization_mean": round(sum(vals) / len(vals), 4)} if vals else {}
except ImportError:
